<script>
    ...
    const input__add_categories = document.querySelector('#kt_tagify_add_category');
...
<script>
    ...
    //tag category
    const tag__add_category = new Tagify(input__add_categories, {
        ...options,
        whitelist: data_categories
    });
    tag__add_category.on('add', function(e) {
        if (e.detail.data.name === e.detail.data.value) {
            tag__add_category.removeTags(e.detail.tag);
        }
    });
</script>
